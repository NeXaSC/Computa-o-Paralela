...
void atualiza_borda(int **borda) {
    ...
    #pragma omp parallel for collapse(2) reduction(+:vizinho) shared(borda, nova_borda) private(i, j)
    for (i = 0; i < LARGURA; i++) {
        for (j = 0; j < ALTURA; j++) {

            vizinho = count_vizinhos(borda, i, j, LARGURA, ALTURA);  // Conta o número de vizinhos vivos
        ...
        // Obtem o número de threads neste momento
        /*
        int num_threads = omp_get_num_threads();
        printf("Iteração (%d,%d) está usando %d threads\n", i, j, num_threads);
        */
        }
    }
